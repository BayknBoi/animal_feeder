...
describe("feeding animals", () => {
  test("Alex likes the food we expect", () => {
    ...
    badFood.forEach((food) =>{
        exper(alex.feed(food)).toBo("Alex the Lion doesn't like ${food}!")
    })
  test("Marty likes the food we expect", () => {
    ...
    badFood.forEach((food)=> {
        expect(marty.feed(food)).toBe("Marty the Zebra doesn't likes ${food}!")
    })
  ...
  badFood.forEach((food) => {
    expect(melman.feed(food)).toBe(`Melman the Giraffe doesn't like ${food}!`)
  })
  ...
  badFood.forEach((food) => {
    expect(gloria.feed(food)).toBe(`Gloria the Hippo doesn't like ${food}!`)
  })
});
